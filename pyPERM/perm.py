...
def build_decision_tree(items, full_categories, priorities):
    ...
    def filter_items(items, conditions):
        return [
            item
            for item in items
            if all(cond in item.attributes for cond in conditions.values())
        ]
    ...
    def build_sub_branches(parent_items, conditions, remaining_cats, top_category):
        ...
        if not remaining_cats:
            return [
                {"attr_value": item.name, "sub_branches": []} for item in parent_items
            ]
        ...
        if not needs_splitting(parent_items, current_cat):
            return build_sub_branches(
                parent_items, conditions, remaining_cats[1:], top_category
            )

        for full_value in sub_values:
            if full_value in seen_values:
                continue
            sub_items = filter_items(
                parent_items, conditions | {current_cat: full_value}
            )
            if sub_items:
                sub_branch = {"attr_value": full_value, "sub_branches": []}
                sub_branch["sub_branches"] = build_sub_branches(
                    sub_items,
                    conditions | {current_cat: full_value},
                    remaining_cats[1:],
                    top_category,
                )
                            ...
                            new_branches.append(
                                {
                                    "attr_value": "ELSE:",
                                    "sub_branches": [
                                        {"attr_value": item_name, "sub_branches": []}
                                    ],
                                }
                            )
    ...
    for group in priorities["groups"]:
        ...
        for full_value in top_values:
            ...
            if items_with_value:
                ...
                branch["sub_branches"] = build_sub_branches(
                    items_with_value,
                    {top_cat: full_value},
                    group["categories"][1:],
                    top_cat,
        ...
        if format == "text":
            ...
            if indent == 0:  # Shouldn’t happen in branches, but for safety
                ...
                if not node["sub_branches"]:  # Leaf node
                    print(
                        f"{indent_str}  => gemini://localhost/item/{node['attr_value']} {node['attr_value']}"
                    )
                else:  # Decision point
...
def main():
    ...
    output_format = (
        "gemtext" if len(sys.argv) == 5 and sys.argv[4] == "--gemtext" else "text"
    )
